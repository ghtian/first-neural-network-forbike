...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_error = error * self.weights_hidden_to_output.flatten()
            ...
            delta_weights_i_h += self.lr * hidden_error_term * X[:, None] / n_records
            # Weight step (hidden to output)
            delta_weights_h_o += self.lr * output_error_term * hidden_outputs[:, None] / n_records
...
# 
# 模型的预测效果一般。
# 观察发现，Validation loss在学习的中后期一直徘徊于0.37与0.38之间，很大的可能性是误差梯度下降至局部低谷，无法越过局部山峰。
